...
/** One‑time loader for <model-viewer> (SSR‑safe). */
...
/**
 * Notes:
 *  • Wrapper shrink‑wraps by default (inline‑block) so the FullscreenModal can
 *    drive size/scale. In 1×1 preview tiles we override this via CSS:
 *      .preview-1x1 > div[data-zu-media="video"] { inset:0; width/height:100% !important; }
 *  • Forward the ref to <video> so FullscreenModal can read videoWidth/Height.
 *  • Hide native fullscreen PiP; keep transport bar visible.
 */
...
function RenderMediaRaw({
  ...
  // PDFs/HTML/text – sandboxed iframe by default
...
/* What changed & why: r788
   • Restored clean file (removed bad merge artifacts; fixed import path).
   • Let CSS force video wrapper to fill in .preview-1x1 tiles (no crop).
   • Kept fullscreen modal behavior: ref on <video>, shrink-wrap by default. */
