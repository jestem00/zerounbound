/*─────────────────────────────────────────────────────────────
  ...
  Rev :    r7    2025‑08‑19 UTC
  Summary(of what this file does): Wallet‑scoped "My Listings" grid.
           Mirrors the explore pipeline: derives the correct TzKT
           `/v1` base per active network, fetches the seller’s active
           on‑chain listings, drops stale ones via FA2 balance checks,
           prefetches token metadata + collection names in batches,
           and renders TokenListingCard with full props so names,
           previews, and bars render reliably.
──────────────────────────────────────────────────────────────*/
...
/** normalize a raw listing row from any of our marketplace view shapes */
function normalizeRow(r = {}) {
  ...
  const active   = (typeof r.active === 'boolean' ? r.active
                   : (typeof r.accepted === 'boolean' ? !r.accepted : true));
...
/** batch: resolve collection names for a set of KT1s via `/v1/contracts` */
async function fetchContractNames(TZKT, contracts) {
  const nameMap = new Map(); // KT1 -> name
...
export default function MyListingsPage() {
  ...
  // Derive active network → correct TzKT `/v1` base (callers MUST NOT add `/v1` again)
  const net = useMemo(() => {
  ...
  const TZKT = useMemo(() => tzktBase(net), [net]); // returns host **with** `/v1` suffix. :contentReference[oaicite:0]{index=0}
  ...
  const assemble = useCallback(async (rawRows) => {
    // 1) normalize + basic guards
    const base = (Array.isArray(rawRows) ? rawRows : []).map(normalizeRow)
      .filter((l) => isKt1(l.contract) && Number.isFinite(l.tokenId) && l.active && Number(l.amount) > 0);

    if (!base.length) return [];

    // 2) stale‑listing guard — verify seller still has >= amount (TzKT balances)
    // NOTE: filterStaleListings batches per token group inside the helper. :contentReference[oaicite:1]{index=1}
    const filtered = await filterStaleListings(
      toolkit,
      base.map((l) => ({
        nftContract: l.contract,
        tokenId: l.tokenId,
        seller,
        amount: l.amount,
      ...
      .catch(() => base)
      .then((arr) => (arr[0]?.__src ? arr.map((x) => x.__src) : arr));

    if (!filtered.length) return [];

    // 3) group for metadata/name prefetch
    ...
    // 4) batch‑fetch: token metadata + collection names using the active TZKT base
    const names = await fetchContractNames(TZKT, [...byContract.keys()]);
    const out = [];
    for (const [kt, idSet] of byContract.entries()) {
      const ids = [...idSet];
      const metaMap = await fetchTokenMetaBatch(TZKT, kt, ids);

      for (const id of ids) {
        const mdEntry = metaMap.get(String(id)) || {};
        const md   = mdEntry.metadata || {};
        const supp = Number(mdEntry.totalSupply ?? 0);

        // Accept cards that have either a renderable preview OR at least valid metadata & non‑zero supply
        if (detectHazards(md).broken) continue;
        if (supp === 0) continue; // non‑zero supply guard as on explore pages. :contentReference[oaicite:2]{index=2}

        // pick a price for (kt,id) — *lowest* among seller’s listings (stable enough)
        let priceMutez = Infinity;
        for (const l of filtered) {
          if (l.contract === kt && Number(l.tokenId) === Number(id)) {
            if (Number.isFinite(l.priceMutez) && l.priceMutez < priceMutez) priceMutez = l.priceMutez;
          }
        }
        if (!Number.isFinite(priceMutez)) priceMutez = 0;

        // preview not strictly required for My Listings (card has fallbacks), but pass metadata for names.
        out.push({
          contract: kt,
          tokenId: id,
          priceMutez,
    ...
    // stable sort by tokenId desc to feel recent‑first (same heuristic as explore) :contentReference[oaicite:3]{index=3}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // Fetch on‑chain seller‑scoped listings (active‑leaning views, but we normalize anyway)
        const rows = await fetchOnchainListingsForSeller({ toolkit, seller }).catch(() => []);
        if (cancel) return;
...
/* What changed & why (r7):
   • Fixed “blank cards” by prefetching token metadata + collection names via
     the correct, network‑aware TzKT base (tzktBase(net) returns a base that
     already includes `/v1`, do not append again) and passing them into
     TokenListingCard (names, previews now render). :contentReference[oaicite:4]{index=4}
   • Eliminated “tzktBase is not a function” by importing tzktBase from
     utils/tzkt.js (not core/net). Parity with explore pages. :contentReference[oaicite:5]{index=5}
   • Kept the stale‑listing guard (seller FA2 balance via TzKT) used in explore
     listings so dead entries never render. :contentReference[oaicite:6]{index=6}
   • Styled‑components import normalized (`styledPkg` shim) as on explore
     pages, avoiding “styled is not a function” errors across SSR. */
/* EOF */
