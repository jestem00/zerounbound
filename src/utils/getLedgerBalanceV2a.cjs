...
module.exports = async function getLedgerBalanceV2a({ tzktBase, contract, tokenId, owner }) {
  const url = `${tzktBase}/v1/tokens/balances?token.contract=${contract}&token.tokenId=${tokenId}&account=${owner}`;
...
/* What changed & why: Added helper to fetch ledger balance for v2a contracts
   using the TzKT API. */
